package by.bsuir.shop.model.dao.impl;
...
import by.bsuir.shop.model.exception.ProductNotFoundException;
import by.bsuir.shop.model.pool.ConnectionPool;
import by.bsuir.shop.model.mappers.ProductListRowMapper;
import by.bsuir.shop.model.mappers.ProductRowMapper;
...
public class JdbcProductDao implements ProductDao {
    ...
    public Product getProduct(Long id) {
        ...
        try {
            ...
            PreparedStatement preparedStatement = connection.prepareStatement("select * from cosmetics where id=?");
    ...
    private String getFindProductsQuery(String[] words, SortType type, SortOrder order) {
        StringBuilder query = new StringBuilder("select * from cosmetics where stock > 0");
        if (words.length > 0) {
