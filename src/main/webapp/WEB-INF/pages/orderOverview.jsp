...
<jsp:useBean id="order" class="by.bsuir.shop.model.order.Order" scope="request"/>
<tag:master pageTitle="Product data">
    ...
    <table border="1px">
        ...
        <c:forEach var="item" items="${order.items}" varStatus="status">
            <tr>
                <td>
                    <img class="cosm-img" src="${item.product.imageUrl}">
                </td>
                <td><a href="${pageContext.servletContext.contextPath}/controller?command=PRODUCT_DETAILS&id=${item.product.id}">${item.product.name}</a></td>
                <td>
                    ${item.quantity}
                </td>
                <td class="price">
